...
function Window({ openCart,   onClickClosedWindow }) {
    ...
    const handleAddToCart = () => {
        const productToAdd = {...windowItem, quantity: quantity, size: textList};
                    ...
                    <div className="windowAll">
                            ...
                            <div className="image-indicators">
                                ...
                                <div className="circle">
                                    <input
                                        ...
                                        height={50}
                                        type="button"
                                        className={isButtonSelected(1) ? "input_circle" : "input_circle"}
                                        onClick={() => handleImageChange(1)}
                                    />
                                    <input
                                        type="button"
                                        className={isButtonSelected(2) ? "input_circle" : "input_circle"}
                                        onClick={() => handleImageChange(2)}
                                    />
                                    <input
                                        type="button"
                                        className={isButtonSelected(3) ? "input_circle" : "input_circle"}
                        ...
                        <div className="window__right">
                            <div className="window__right-text">
                                <p>{windowItem.name}</p>
                                <p>{windowItem.originalPrice}₴</p>
                            </div>
                            <div className="window__right-size">
                                <p>Black</p>
                                <button className="window__right-size-btn cu-p" onClick={() => setOpen(!open)} >
                                    <p>{textList}</p>
                                    <img width="17" height="17" onClick={() => setOpen(!open)} src={open ? arrow_top : arrow_down} alt=""/>
                                </button>
                                {open && (
                                    <div className="window__right-size-open">
                                        {list.map((sort, index) => (
                                            <li key={index} onClick={() => onClickSorting(index)} className={selected === index ? "active" : "color-red"}>
                                                {sort}
                                            </li>
                                        ))}
                                    </div>
